          ...
          <div className="tw-p-10 tw-border-b tw-border-white/5 tw-flex tw-justify-between tw-items-center tw-bg-gradient-to-r tw-from-indigo-600/10 tw-to-transparent">
            <h2 className="tw-text-4xl tw-font-black tw-flex tw-items-center tw-gap-6 tw-tracking-tighter tw-text-white">
              <div className="tw-w-16 tw-h-16 tw-bg-indigo-600 tw-rounded-3xl tw-flex tw-items-center tw-justify-center tw-shadow-lg tw-shadow-indigo-500/20">
                <Fingerprint size={32} className="tw-text-white" />
              </div>
              <div className="tw-flex tw-flex-col">
                <span className="tw-text-3xl tw-line-height-none">{user ? 'Identity Sync' : 'IAM Onboarding'}</span>
                <span className="tw-text-xs tw-text-indigo-400 tw-font-bold tw-uppercase tw-tracking-[0.2em] tw-mt-1">Personnel Authorization Hub</span>
              </div>
            </h2>
          ...
          <div className="tw-p-12 tw-space-y-10 tw-overflow-y-auto tw-max-h-[60vh]">
            ...
            <section className="tw-space-y-6">
              <h3 className="tw-text-xs tw-font-black tw-text-slate-500 tw-uppercase tw-tracking-[0.2em] tw-flex tw-items-center tw-gap-4">
                <Fingerprint size={14} className="tw-text-indigo-500" /> Identity Core
              </h3>
              <div className="tw-grid tw-grid-cols-2 tw-gap-6 tw-mt-4">
                <div className="tw-space-y-3">
                  ...
                  <input 
                    ...
                    className="tw-w-full tw-bg-slate-800/50 tw-border tw-border-white/10 tw-p-5 tw-rounded-3xl tw-outline-none focus:tw-border-indigo-500 tw-transition-all tw-text-white tw-font-mono" 
                ...
                <div className="tw-space-y-3">
                  ...
                  <input 
                    ...
                    className="tw-w-full tw-bg-slate-800/50 tw-border tw-border-white/10 tw-p-5 tw-rounded-3xl tw-outline-none focus:tw-border-indigo-500 tw-transition-all tw-text-white tw-font-bold" 
              ...
              <div className="tw-space-y-3">
                <label className="tw-text-[11px] tw-font-black tw-text-slate-500 tw-uppercase tw-tracking-widest tw-ml-1 tw-flex tw-items-center tw-gap-2">
                  <Mail size={12} className="tw-text-indigo-500" /> Communication Channel (IAM Email)
                </label>
                <input 
                  required type="email" value={formData.email}
                  onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                  className="tw-w-full tw-bg-slate-800/50 tw-border tw-border-white/10 tw-p-5 tw-rounded-3xl tw-outline-none focus:tw-border-indigo-500 tw-transition-all tw-text-white" 
            ...
            <section className="tw-space-y-6">
              ...
              <div className="tw-grid tw-grid-cols-2 tw-gap-4">
                {['System Admin', 'Security Ops', 'Support Pro', 'Standard Operator'].map(role => (
                   <button
                    key={role}
                    type="button"
                    onClick={() => setFormData({ ...formData, role })}
                    className={`tw-p-5 tw-rounded-[24px] tw-border tw-text-xs tw-font-black tw-uppercase tw-tracking-widest tw-transition-all tw-text-left tw-flex tw-justify-between tw-items-center ${formData.role === role ? 'tw-bg-indigo-600 tw-border-indigo-500 tw-text-white tw-shadow-lg tw-shadow-indigo-500/20' : 'tw-bg-white/5 tw-border-white/5 tw-text-slate-500 hover:tw-border-white/20'}`}
                   >
                     {role}
            ...
            <div className="tw-p-8 tw-bg-indigo-500/10 tw-border tw-border-indigo-500/20 tw-rounded-[32px] tw-flex tw-gap-6 tw-items-start shadow-inner">
